...
Though `pip`, you can add more packages to your python/Jupyter. The following
are examples that apply to python3
...
submission, such as `atlas/20210403`. You will also need to use `pip3` to
install `dask-jobqueue` and `distributed`:
...
1. Use `pip3` to install `bokeh` (in order to start DASK dashboard
   automatically). This is a one time task.
...
`python3 -m pip install pyhf` is the easiest way (but not via Conda). We want to
show how to do this in an Conda environment, and make it available in Jupyter.
